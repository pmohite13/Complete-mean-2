...
class VolunteersRepository {
    ...
    getVolunteerByUser(user, callback) {
        console.log('*** VolunteersRepository.getVolunteerByUser');
        Volunteer.find({ 'user.email': user.email }, (err, volunteer) => {
            if (err) {
    ...
    insertVolunteer(body, callback) {
        ...
        let user = { 'firstName': body.user.firstName, 'lastName': body.user.lastName, 'email': body.user.email, 'phone': body.user.phone, 'password': '' };
        console.log(body);
